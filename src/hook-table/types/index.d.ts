...
type CaptionAlignment = 'top-left' | 'top-center' | 'top-right' | 'bottom-left' | 'bottom-center' | 'bottom-right';
export type TableRecord = Record<PropertyKey, unknown>;
export type FormatOptions = Record<string, (value: never) => ReactNode>;
type SubrowActions = {
    closeSubrow?: () => void;
};
type ColumnChildren<T extends TableRecord = TableRecord> = ReactNode | ((rowData: T, subrowActions: SubrowActions) => ReactNode);
type NestedKeyOf<T, K = keyof T> = K extends keyof T & (string | number) ? `${K}` | (T[K] extends object ? `${K}.${NestedKeyOf<T[K]>}` : never) : never;
export type ValueFormatKey<F extends FormatOptions = FormatOptions> = keyof F | undefined;
/**
 * Column props
 */
type ColumnPropsWithAccessor<T extends TableRecord = TableRecord, F extends FormatOptions = FormatOptions> = {
    /**
...
export type ColumnProps<T extends TableRecord = TableRecord, F extends FormatOptions = FormatOptions> = {
    /**
